...
const DEFAULT_GLOW_COLOR = '24, 208, 219';
...
export const ParticleCard = ({
  ...
  return (
    <div
      ref={cardRef}
      className={`${className} magic-particle-container ${enableBorderGlow ? 'magic-border-glow' : ''}`}
...
export const GlobalSpotlight = ({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = e => {
      ...
      const cards = gridRef.current.querySelectorAll('.magic-border-glow');
    ...
    const handleMouseLeave = () => {
      gridRef.current?.querySelectorAll('.magic-border-glow').forEach(card => {
        card.style.setProperty('--magic-glow-intensity', '0');
      });
